...
$(document).ready(function () {
  dataTable = $("#notas_de_venta_tabla").DataTable({
    ordering: false,
    language: {
...
function getNotas(dataTable) {

  $.ajax({
    url: 'get/getVentas.php',
    method: 'POST',
    data: {
      action: 'get_notas_de_venta'
    },
    dataType: 'json',
    success: function (data) {
      ...
      $.each(data, function (index, result) {
        ...
        var estado = "";
        if (result.estado == "1") {
          estado = "<button type='button' class='btn btn-success'>Registrado</button>"
        ...
        var newRow = '<tr>' +
          ...
          '<td class="align-middle text-center">' + result.operacion_exonerada + '</td>' +   
          '<td class="align-middle text-center">' + result.operacion_ina + '</td>' +   
